      ...
      <section className="relative z-10 border border-emerald-500/30 bg-gradient-to-r from-slate-950/90 via-emerald-900/30 to-emerald-700/20 backdrop-blur-xl p-10 md:p-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {COPY.caseStudies.cards.map((cs) => (
            <article key={cs.client} className="border border-white/15 bg-black/25 p-8 md:p-10">
              <div className="text-sm font-black tracking-[0.3em] text-emerald-400 uppercase mb-4">{cs.badge}</div>
              <div className="text-6xl font-black text-emerald-400 mb-5 leading-none">{cs.outcome}</div>
              <h3 className="text-xl md:text-2xl font-black text-white mb-4 uppercase leading-tight">{cs.client}</h3>
              <p className="text-white/85 text-[17px] leading-relaxed font-semibold">{cs.problem}</p>
            </article>
          ))}
        </div>
      </section>
